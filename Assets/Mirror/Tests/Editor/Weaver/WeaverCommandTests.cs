...
namespace Mirror.Weaver.Tests
{
    public class WeaverCommandTests : WeaverTestsBuildFromTestName
    {
        ...
        {
            Assert.That(weaverErrors, Contains.Item("CmdFunction has invalid parameter connection, Cannot pass NetworkConnections. Instead use '[SenderConnection] NetworkConnection conn = null' to get the sender's connection on the server (at System.Void WeaverCommandTests.ErrorForOptionalNetworkConnectionThatIsNotSenderConnection.ErrorForOptionalNetworkConnectionThatIsNotSenderConnection::CmdFunction(Mirror.NetworkConnection))"));
        }

        [Test]
        public void ErrorForNetworkConnectionThatIsNotSenderConnection()
        {
            Assert.That(weaverErrors, Contains.Item("CmdFunction has invalid parameter connection, Cannot pass NetworkConnections. Instead use '[SenderConnection] NetworkConnection conn = null' to get the sender's connection on the server (at System.Void WeaverCommandTests.ErrorForNetworkConnectionThatIsNotSenderConnection.ErrorForNetworkConnectionThatIsNotSenderConnection::CmdFunction(Mirror.NetworkConnection))"));
        }
